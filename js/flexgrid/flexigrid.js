...
(function ($) {
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			errormsg: 'Hubo un error al extraer los datos',
			usepager: false,
			nowrap: true,
			page: 1, //current page
			total: 1, //total pages
			...
			outof: 'de',
			findtext: 'Buscar',
			procmsg: 'Extrayendo datos, por favor espere...',
			query: '',
			qtype: '',
			nomsg: 'No hay datos',
		...
		var g = {
			...
			doSearch: function ()
			 {
				p.query = $('input[name=q]').val();
				p.qtype = $('select[name=qtype]').val();
				p.newp = 1;
				this.populate();
			},
		...
		if (p.usepager) {
			...
			if (p.searchitems) 
			{
				...
				$(g.tDiv).append("<div class='sDiv2'><img src='/images/flexgrid/magnifier.png' align='absmiddle'><input type='text' value='" + p.query +"' size='30' name='q' class='qsbox' /></div>");
				//Split into separate selectors because of bug in jQuery 1.3.2
				$('input[name=q]', g.tDiv).keydown(function (e) {
					if (e.keyCode == 13) {
						g.doSearch();
					}
				});
