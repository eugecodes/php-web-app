...
var MooFlow = new Class({
	...
	createMooFlowElement: function(counter, i){
		...
		obj['con'] = new Element('div').addClass('myBg').inject(obj['div']);
...
Element.implement({
	reflect: function(arg){
		i = arg.img.clone();
		i = new Element('span',{}); //Esto es un hack para evitar que se "adopte" el elemento "i" con el reflejo de la imagen...lo reemplazamos por un span.
		...
		if(Browser.Engine.trident){
			...
			var ret = new Element('img',{
						'class'	:'ie_img',
						'src'	:'/images/flow_imagebg.png'
					  });
					  
			var ie_canvas = new Element('div',{
						'class'	:'canvasbg_ie'
					  });
					  
			return Element(ie_canvas).adopt(ret);		  
			//return ret;
		} else {
			var ret = new Element('img',{
						'class'	:'ie_img',
			...
			var can = new Element('div').setProperties({'width':arg.width, 'height':arg.height, 'class':'canvasbg'});
			if(can.getContext){
