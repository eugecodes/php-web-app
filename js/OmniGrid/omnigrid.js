...
var omniGrid = new Class({
	...
	loadData: function (url)
	{
		...
		if(this.options.dataProvider)
		{
			...
			var request = new Request.JSON({
				method: 'post',
				url: url,
				data: 'page='+param.page+'&perPage='+param.perPage+'&sortOn='+param.sortOn+'&sortBy='+param.sortBy+''+this.options.dataProvider
			});
	...
	setColumnProperty: function(columnName, property, value){
		...
		{
			if ( cmu[i].dataIndex.toString().toLowerCase() == columnName )
			{
	...
	sort: function(index, by){
		...
		if (by != null)
			el.addClass(by.toString().toLowerCase());
		...
		{			
			for (var i=0; i<this.options.data.length; i++)
			{
				...
				for (var c=0; c<this.options.columnModel.length; c++)
				{
					...
					if(dat[columnModel.dataIndex] != null && dat[columnModel.dataIndex].toString().toLowerCase().indexOf(key) > -1)
					{
